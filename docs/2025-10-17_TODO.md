...
- [ ] Document the layered model (systems → plugins → UI)
  - [ ] Add `docs/ARCHITECTURE.md` describing roles of `crates/plugin-api`, `crates/tui`, and `src/` (app wiring)
  - [ ] Link `docs/ARCHITECTURE.md` from `README.md` and briefly explain how built-in plugins are registered
  - Acceptance: New doc exists, README has a short “Architecture” section with link

- [ ] Clarify “systems” vs “plugins” in docs
  - [ ] Add a short section in `docs/ARCHITECTURE.md#Layers` that defines systems (infra: search worker, filesystem indexer) and plugins (UX tabs built on systems)
...
- [ ] Introduce feature flag `capabilities` in `crates/plugin-api/Cargo.toml`
  - [ ] Add optional `Capability` enum and `PluginBundle` trait under `#[cfg(feature = "capabilities")]`
  - [ ] Ensure `lib.rs` re‑exports are gated and don’t affect current API
  - Acceptance: `cargo check` passes with and without the feature

- [ ] Prototype bundle registration (internal only)
  - [ ] Add `PluginBundle` sampling to registry behind the same feature flag (no callers yet)
  - [ ] Keep `SearchPluginRegistry` behavior stable if the feature is off
  - Acceptance: No behavior changes without the feature enabled

- [ ] Add a minimal example bundle
  - [ ] Create `examples/capability_demo.rs` (feature‑gated) that returns a single `Capability::SearchTab`
...
- [ ] Add `thiserror` to the app crate (binary) and define `ConfigError`
  - [ ] Add a validation pass after resolution in `src/settings/resolved.rs`
  - [ ] Include key, value (where sensible), and source (file/env/flag) in diagnostics
...
- [ ] Coalesce bursty resize events before draw
  - [ ] Drain `Resize` events from `event_rx` just before `terminal.draw` in `src/ui/runtime.rs`
  - Acceptance: No flicker; stable frame rate under repeated terminal resizes

- [ ] Guard against overdraw with bounded per‑tick work (keep current bounds)
  - [ ] Document rationale at top of `src/ui/indexing.rs` (MAX_UPDATES / MAX_TIME)
...
- [ ] Introduce typed errors where external callers benefit most
  - [ ] Add error enums in `crates/plugin-api` for public fallible APIs (if any)
  - [ ] Keep `anyhow` in the binary for ergonomic top‑level handling
