...
- Search within docs: `ripdoc print [target] --search <term>` (combine with `--search-spec name,doc,path,signature` and `--direct-match-only`/`-d` to avoid expanding parents)
- OR searches: `ripdoc print gix --search "init|clone|fetch|remote|config"`
- Raw data for tooling: `ripdoc raw [target]` → JSON rustdoc
- Fetch README: `ripdoc readme [target]`
